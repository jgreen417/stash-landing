          ...
          <div className="flex flex-col sm:flex-row items-start gap-5 md:gap-7">
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                ...
                {/* ── Tier 2: Foundation features (Alternating rows) ── */}
          ...
          {tier2.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                className={`p-6 md:p-8 rounded-2xl bg-white border border-border flex flex-col md:flex-row items-center gap-6 md:gap-8 ${isEven ? "md:flex-row-reverse" : ""}`}
              >
                <motion.div
                  variants={iconVariant}
                  initial="hidden"
                  animate={inView ? "show" : "hidden"}
                  className="w-16 h-16 md:w-20 md:h-20 shrink-0 rounded-2xl flex items-center justify-center bg-[hsl(190,70%,25%,0.08)]"
                >
                  <Icon size={32} style={{ color: "hsl(190,70%,25%)" }} />
                </motion.div>
                <div className={`flex-1 ${isEven ? "md:text-right" : "md:text-left"}`}>
                  ...
                  <span className="inline-block mt-4 text-sm font-semibold" style={{ color: "hsl(190,70%,25%)" }}>
                    {item.detail} &rarr;
                  </span>
                </div>
              </motion.div>
